...
const EDGE_SIZE = 22; // px from edge that counts as drag zone (larger for touch)
  ...
  return (
    <>
      {node.connections.map((targetId) => {
        ...
        // Bezier control points for a soft curve
        const cx1 = x1 + (x2 - x1) * 0.5;
        const cx2 = x2 - (x2 - x1) * 0.5;
        const pathD = `M ${x1} ${y1} C ${cx1} ${y1}, ${cx2} ${y2}, ${x2} ${y2}`;
        const connId = `grad-${node.id}-${targetId}`;
        return (
          <svg
          ...
          >
            ...
            />
            {/* Endpoint dots */}
            <circle cx={x1} cy={y1} r="4" fill={node.color} opacity="0.9" />
            <circle cx={x2} cy={y2} r="4" fill={target.color} opacity="0.9" />
          </svg>
        );
      })}
